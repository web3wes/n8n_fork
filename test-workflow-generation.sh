...
for i in "${!PROMPTS[@]}"; do
    ...
    if echo "$WORKFLOW_RESPONSE" | jq -e '.code' >/dev/null 2>&1; then
        ERROR_CODE=$(echo "$WORKFLOW_RESPONSE" | jq -r '.code')
        ERROR_MESSAGE=$(echo "$WORKFLOW_RESPONSE" | jq -r '.message')
        echo "❌ Workflow $WORKFLOW_NUM generation failed (Error $ERROR_CODE)"
        echo "   Error: $ERROR_MESSAGE"
    elif echo "$WORKFLOW_RESPONSE" | jq -e '.nodes' >/dev/null 2>&1; then
